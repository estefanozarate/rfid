 ...
 * ──────────────────────────────────────────────────────────
 * Pantalla 1: Bienvenida a stamping.io RFID Reader.
 *
 * - Título y branding de stamping.io
 * - Botón "Comenzar" → navega a ScannerScreen
 * - Indicador visual de qué tecnologías soporta
 */
...
// ─── Íconos SVG inline como componentes View ───────────────
// (evitamos dependencia de vector-icons para mantener bundle ligero)
const NfcIcon = ({ color = Colors.accent, size = 48 }) => (
  <View style={{ width: size, height: size, alignItems: 'center', justifyContent: 'center' }}>
    {/* Representación simplificada de señal NFC con View anidados */}
    <View style={{
      width: size * 0.55, height: size * 0.55,
      borderRadius: (size * 0.55) / 2,
      borderWidth: 2, borderColor: color, opacity: 0.9,
      position: 'absolute',
    }} />
    <View style={{
      width: size * 0.75, height: size * 0.75,
      borderRadius: (size * 0.75) / 2,
      borderWidth: 1.5, borderColor: color, opacity: 0.5,
      position: 'absolute',
    }} />
    <View style={{
      width: size * 0.95, height: size * 0.95,
      borderRadius: (size * 0.95) / 2,
      borderWidth: 1, borderColor: color, opacity: 0.25,
      position: 'absolute',
    }} />
    <View style={{
      width: 8, height: 8,
...
const WelcomeScreen = ({ navigation }) => {
  ...
  return (
    <SafeAreaView style={styles.safe}>
      ...
      {/* Fondo con puntos de cuadrícula decorativos */}
      ...
      <View style={styles.container}>
        ...
        <Animated.View style={[styles.btnWrapper, fadeUp(btnAnim), { transform: [{ scale: btnScale }] }]}>
          <TouchableOpacity
            ...
            accessibilityLabel="Comenzar a escanear tags RFID"
